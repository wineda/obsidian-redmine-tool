...
- **表示モードの切り替え**: ガントチャート / チケット一覧テーブルをツールバーで切り替え(選択は記憶)
  ...
  - コンパクトな行高で一覧性を重視。**文字サイズは設定で8〜16pxに調整可能**(テーブル・ガント共通。行の高さ・バッジ類も連動、既定11px)
...
- **全体予定**: Redmineとは独立した予定をプラグイン内で管理し、ガントの最上段の「全体予定」帯に表示
  - ツールバーのカレンダーアイコンから追加・編集・削除(予定名・期間・状態)
  - **複数日の予定はタイトル入りのブロック**、**1日の予定は▼マーカー+タイトル**で表示
  - 期間が重なる予定は自動でレーンを分けて配置(重ならない)
  - **時間軸ヘッダーと全体予定帯は縦スクロールしても上部に固定**
...
- 専用ビューでのガントチャート表示
  ...
  - 親チケット配下に子チケットをインデント表示。担当者はバッジ(チップ)表示
...
| 既定のスケール | 日 / 週 / 月 |
| 文字サイズ | テーブル・ガント共通。8〜16pxのスライダー(既定11)。行の高さも連動し、変更は開いているビューに即時反映 |
...
Redmineのチケットとは別に、フェーズやマイルストーンなどの大枠の予定をプラグイン内で管理できます。ツールバーのカレンダーアイコンで編集画面を開き、予定名・開始日・終了日・状態(未着手/進行中/完了)を設定します。ガント最上段の「全体予定」帯に、複数日の予定はタイトル入りブロック、1日の予定は▼マーカー+タイトルとして、状態ごとの色(グレー/オレンジ/緑)で表示されます。期間が重なる予定は自動でレーンが分かれます。Redmineには一切書き込まれません。
