...
| プロジェクト | 既定表示のプロジェクト識別子または ID。空欄で閲覧可能な全チケット |
| 完了チケットを含める | 終了ステータスのチケットも表示 |
| 既定のスケール | 日 / 週 / 月 |
| 表示フィルタ | ツールバーで切り替える表示条件を複数登録できる(下記) |

### 表示フィルタ

| 種類 | 値の指定 | 動作 |
|---|---|---|
| プロジェクト | プロジェクト識別子または ID | そのプロジェクト+サブプロジェクトのチケット |
| 親チケット配下 | チケット番号(例: `1234`) | 指定チケットと、その配下ツリー全体。`parent_id` フィルタで階層を1段ずつ辿って収集 |
...
リボンのガントチャートアイコン、またはコマンドパレットの「Redmine Gantt: ガントチャートを開く」でビューを開きます。ツールバーの更新ボタンで再取得、左のドロップダウンで表示フィルタ、右のドロップダウンでスケールを切り替えられます。選択したフィルタは記憶され、次回も同じ表示で開きます。
