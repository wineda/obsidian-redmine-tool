...
| **2.14(実装済み)** | 全体予定の表示刷新: 左ペインは「全体予定」固定ラベル、複数日はタイトル入りブロック、1日は▼マーカー+タイトル。重なる予定は自動レーン分割(1日予定はラベル幅も考慮) |
| **2.15(実装済み)** | ガントの時間軸ヘッダー+全体予定帯を縦スクロール時に上部固定(sticky)。描画を上部固定SVGとチケット行SVGに分割し、左ペイン幅リサイズは両方に同期 |
